...
Leftover-vs-d on the hex disk is at a stop. Two-aether pass-through earned (lab 30). Continue-after-LCM-hit earned (lab 31). Packing SOURCE from MI leftover shadow earned (lab 32). Sitting halo with that source is not earned (lab 33: CRUST at T=0). Lab 34 earns squeeze-density-law=1/r² through r~10, while superposition is not earned and is weakly constrained by one live probe. The source is not a sitting halo. Still no flatten, SN clock, Hubble number, or λ scan. Do not add T to make a halo sit. Do not scan λ looking for a flatten. SN dilation stays parked; do not add a clock well or c(E). No next lab until Glen says go.
