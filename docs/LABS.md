...
- A sitting diffuse LCM halo. T = 0 packing is a crust; in 2D it blows off. Do not add T. Lab 32 earned a packing SOURCE (inward leftover on unpacked LCM from the MI shadow) but the sitting halo is still not earned.
...
Leftover-vs-d on the hex disk is at a stop. Two-aether pass-through earned (lab 30). Continue-after-LCM-hit earned (lab 31). Packing SOURCE from MI leftover shadow earned (lab 32) without making LCM a leftover wall. Sitting halo still not earned. Still no flatten. Still no SN clock. Do not scan λ looking for a flatten. SN dilation stays parked; do not add a clock well or c(E). No next lab until Glen says go.
