...
Leftover-vs-d on the hex disk is at a stop. Two-aether pass-through is earned (lab 30). Continue-after-LCM-hit is earned (lab 31): leftover can heat LCM without becoming a wall; leftover F is still the MI-only kernel. Labs 28–29 empty window was leftover rain stopping on LCM. Still no flatten. Still no SN clock. Heat is a ledger, not a leftover force on LCM. Do not scan λ looking for a flatten. SN dilation stays parked; do not add a clock well or c(E). No next lab until Glen says go.
