...
- pathorder-twopop+light (local). Lab 56: path-order costs local 1/d² (χ² 1/d²=3.71 vs 1/d=10.50; far d=16 live 2.50σ). Labs 59–60: fail = reclass channel (F_pass0 still 1/d²; F_reclass other far-live). Not earned as preferred local coding; unordered preferred locally (60). Hex path-order still earned (57).
...
1/r dens = 〈u·r̂〉 weight (58). Path-order local fail = reclass channel (59–60). Unordered preferred locally; path-order hex still ok (57). Flatten/SN/T parked. No next lab until Glen says go.
